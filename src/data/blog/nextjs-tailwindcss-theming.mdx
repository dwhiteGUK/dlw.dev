...
Below is an approach to switching between multiple themes using [next themes](https://github.com/pacocoursey/next-themes) and not just light and dark themes. This approach combines next themes, Tailwind CSS and CSS custom properties.
...
- Node (tested with v14, a couple of version earlier would be fine)
- Npm (tested with v7)
...
In the above, I'm using colours provided by Tailwind CSS. If I wanted to add custom colours I'd extend the tailwind.config.js and add them there ensuring our Tailwind CSS config remains the source of truth for our theme. The `:root` is essentially the light theme, notice how the other options match the themes array setup earlier.
...
When the website is first loaded next themes will set the theme attribute on the HTML element to light, `<html data-theme="light">` - if changed that will get updated to the selected theme `<html data-theme="pink">`. When this happens the underlying colour for our custom Tailwind CSS class will get updated and therefore change out colour scheme. The below video shows this in action:

<video controls width="655px" height="464.9px">
